...
function News({category, setProgress}) {
    ...
    useEffect(() => {
        try {
            async function fetchData() {
                let res = await fetch(`https://newsapi.org/v2/top-headlines?country=ca&category=${category}&apiKey=273947c87e4c47a99f1794b7e02e1cd5&page=1&pageSize=9`);
    ...
    const handleNextClick = () => {
        async function fetchData() {
            let res = await fetch(`https://newsapi.org/v2/top-headlines?country=ca&category=${category}&apiKey=273947c87e4c47a99f1794b7e02e1cd5&page=${page+1}&pageSize=${pageSize}`);
            let data = await res.json();
            let article = data.articles;
            setNews(article)
        }
